...
market_data[Date >= start_date & Date <= stop_date & is.na(USGG1M_Return_Pct), ] # Missing day on '2015-05-08'
...
days <- length(avg_vol_days)

## There are 2,518 Days in Average Volatility time period

# very high volatility (VIX >30) 9/15/08 - 5/18/09

start_date <- '2008-09-15'
stop_date  <- '2009-05-18'

market_data[Date >= start_date & Date <= stop_date & is.na(USGG1M_Return_Pct), ] # No Missing
market_data[Date >= start_date & Date <= stop_date & is.na(SPY_Return_Pct)   , ] # No missing
market_data[Date >= start_date & Date <= stop_date & is.na(TLT_Return_Pct)   , ] # No missing

high_vol_days <- market_data[Date >= start_date & Date <= stop_date, Date]
days <- length(high_vol_days)
...
market_data[Date %in% c(range1,range2) & is.na(USGG1M_Return_Pct), ] # No Missing
market_data[Date %in% c(range1,range2) & is.na(SPY_Return_Pct), ]    # No Missing
market_data[Date %in% c(range1,range2) & is.na(TLT_Return_Pct), ]    # No Missing

low_vol_days <- market_data[Date %in% c(range1,range2),Date] 
days <- length(low_vol_days)
...
# Normal Simulation
SPY_sim_returns <- rnorm(2518,SPY_mean,SPY_sd)
...
days  <- 2518              # Days in 10 year time period
...
low_vol_days
x <- avg_vol_days
market_data$growth_return
market_data$balanced_return
market_data$conservative_return
market_data$growth_excess_return
market_data$balanced_excess_return
market_data$conservative_excess_return
...
avg_sim <- replicate(loops, ret.fun(avg_vol_days), simplify = "array")                 # Simulate one year's annual return, 5000 times
avg_vol_sim <-  data.table(t(matrix(unlist(avg_sim), nrow=length(unlist(avg_sim[1])))))     # Force data into data table
...
hi_sim <- replicate(loops, ret.fun(high_vol_days), simplify = "array")                 # Simulate one year's annual return, 5000 times
high_vol_sim  <-  data.table(t(matrix(unlist(hi_sim), nrow=length(unlist(hi_sim[1])))))     # Force data into data table
setnames(high_vol_sim,c('V1','V2','V3','V4', 'V5', 'V6'), 
         c('annual_return_growth', 'annual_return_balanced', 'annual_return_conservative',
           'sharpe_ratio_growth', 'sharpe_ratio_balanced', 'sharpe_ratio_conservative'))

# (iii) low_vol_days simulation and formatting
low_sim <- replicate(loops, ret.fun(low_vol_days), simplify = "array")                 # Simulate one year's annual return, 5000 times
low_vol_sim  <-  data.table(t(matrix(unlist(low_sim), nrow=length(unlist(low_sim[1])))))     # Force data into data table
...
# The sharpe ratio measures risk adjusted returns using the following formula:
# excess return = daily return - risk free rate
# Sharpe ratio = average(excess returns) / std(excess returns)

# done previously:
# market_data$spy_excess_return = (market_data$SPY_Return_Pct - market_data$risk_free_daily_rate)

Sharpe_ratio <- ( mean(result$spy_excess_return) / sd(result$SPY_Return_Pct) ) / (252^(1/2))
Sharpe_ratio
